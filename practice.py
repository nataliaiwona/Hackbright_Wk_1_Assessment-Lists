...
# I tried a list comprehension on this, but I don't think it worked?
...
def halvesies(numbers):
    ...
    halved = [num / 2.0 for num in numbers]
...
def join_strings_with_comma(words):
    ...
    joined_words = ", ".join(word for word in words)
...
def duplicates(items):
    ...
    duplicated_items = [item for item in items if items.count(item) > 1]
    dup_set = set(duplicated_items)
    dup_list = list(dup_set)
    return dup_list

    # This also gave me trouble.
...
def find_letter_indices(words, letter):
    ...
    **DO NOT** use the `list.index()` method.
    ...
    for word in words:
        index = 0
        before_len = len(letters_in_word)

        while index < len(word):
            if word[index] == letter:
                letters_in_word.append(index)
                break
            index += 1

        after_len = len(letters_in_word)

        if before_len == after_len:
            letters_in_word.append(None)

    return letters_in_word

# This took me a very long time! And it seems very chunky.
